...
diesel_trucks = {
    ...
    "ISUZU ELF600": {"cost_initial": 1050000 * 1.16, "km_per_liter": 7, "maintenance_annual": 7000, "capacidad_combustible": 140}
}
...
# Información del camión eléctrico Sany
st.divider()
st.markdown("<h4 style='text-align: center;'>Información del Camión Eléctrico Sany FE601</h4>", unsafe_allow_html=True)
sany_info = {
    "Modelo": "Sany FE601",
    "Capacidad de la batería (kWh)": 84.48,
    "Consumo de energía (% por km)": 2.33,
    "Autonomía por carga completa (km)": 200,
    "Costo inicial (incl. IVA)": 1350000 * 1.16,
    "Costo de mantenimiento anual ($)": 4000,
    "Seguro anual ($)": 53000
}
st.table(pd.DataFrame(sany_info, index=[0]).T)
...
for year in range(1, 5):
    electricity_cost = (cost_per_kwh * electric_data["battery_capacity_kwh"] * annual_kilometers) / electric_data["distance_per_charge_km"]
